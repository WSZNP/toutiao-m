  ...
  methods: {
    async onPost () {
      this.$toast.loading({
        ...
        duration: 0 // 持续时间，默认是2秒,如果为0则持续展示
      })
      try {
        const { data } = await addComment({
          target: this.target.toString(), // 评论的目标id,id可以为文章id或评论的id
          content: this.message, // 评论的内容
          art_id: this.articleId ? this.articleId.toString() : this.articleId// 文章id,对评论回复需要此参数值, 代表所属文章id, 对文章评论无需此参数
        })
        console.log(data)
